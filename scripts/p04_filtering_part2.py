...
plt.figure()
plt.plot(hz, np.abs(fft(signal1) / N)**2, label='signal')
plt.plot(hz, np.abs(fft(noise) / N)**2, label='noise')
plt.legend()
plt.xlim([0, 100])
...
imppow = np.abs(fft(fimpulse, 2 * len(hz)))**2
...
signalX = np.abs(fft(signal1) / N)**2
fdataX = np.abs(fft(fdata) / N)**2
hz = np.linspace(0, fs, N)
...
filterpow = np.abs(fft(filterkern))**2
...
# plot power spectrum
noisepower = np.abs(fft(filtnoise))**2
plt.subplot(122)
...
filter_freqs = [0, lower_bnd * (1 - transw), lower_bnd,
                upper_bnd, upper_bnd + upper_bnd * transw, samprate / 2]
...
filterpow = np.abs(fft(filtkern))**2
...
# plot power spectrum
noiseX = np.abs(fft(noise))**2
fnoiseX = np.abs(fft(fnoise))**2
...
sincXrolloff = (sincX[sincX3db] - sincX[sincXoct]) / \
    (hz[sincXoct] - hz[sincX3db])
butterXrolloff = (butterX[butterX3db] - butterX[butterXoct]
...
# %%
# load data
linedata = loadmat('filtering/lineNoiseData.mat')
data = np.squeeze(linedata['data'])
srate = np.squeeze(linedata['srate'])
...
for fi in range(0, len(frex2notch)):
    ...
    plt.subplot(122)
    plt.plot(np.linspace(0, srate, 10000), np.abs(
        scipy.fftpack.fft(filtkern, 10000))**2)
...
# %%
# load in birdcall
fs, bc = scipy.io.wavfile.read('filtering/XC403881.wav')
...
for fi in range(0, len(frange)):
    plt.plot([time[0], time[-1]], [frange[fi][0], frange[fi][0]],
             colorz[fi] + '--', linewidth=3)
    plt.plot([time[0], time[-1]], [frange[fi][1], frange[fi][1]],
             colorz[fi] + '--', linewidth=3)
